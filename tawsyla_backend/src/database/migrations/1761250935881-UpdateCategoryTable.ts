...
export class UpdateCategoryTable1761250935881 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasDescriptionEn = await queryRunner.hasColumn('categories', 'descriptionEn');
    if (!hasDescriptionEn) {
      await queryRunner.query(
    ...
    const hasDescriptionAr = await queryRunner.hasColumn('categories', 'descriptionAr');
    if (!hasDescriptionAr) {
      await queryRunner.query(
    ...
    const hasDescription = await queryRunner.hasColumn('categories', 'description');
    if (hasDescription) {
      await queryRunner.query(

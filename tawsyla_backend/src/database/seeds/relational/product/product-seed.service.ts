...
import {
  ...
  SizeUnit,
  WeightUnit,
} from '../../../../product/entities/product-variant.entity';
import {
  Company,
  CompanyType,
    ...
    for (let i = 0; i < simplifiedData.length; i += batchSize) {
      ...
      for (const productData of batch) {
        try {
          ...
          const category = this.findMatchingCategory(
            productData.category,
            categoryMap,
          );
          if (!category) {
            console.log(
              `No category found for product ${productData.id}: ${productData.name}`,
            );
            productsSkipped++;
            continue;
          }
          ...
          console.error(
            `Error creating product ${productData.id}:`,
            error.message,
          );
          productsSkipped++;
        }
      ...
      console.log(
        `Processed batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(simplifiedData.length / batchSize)}`,
      );
    }

    console.log(`Successfully created ${productsCreated} products`);
    console.log(
      `Skipped ${productsSkipped} products due to errors or missing data`,
    );
  }
